...
@section('contents')
 <div class="row mb-3" style="border-bottom: 2px solid #219653">
                        <div class="col-md-9 order-sm-1 order-1 col-6 ">
                            <h3 class="title-responsive"><a href="{{route('admin.deposits.index')}}"> <i class="anticon anticon-left text-primary "></i></a> Withdraw Details </h3>
                        </div>
                                        ...
                                        <div class="row">
                                            <div class="text-center text-sm-left col-12 col-sm-3  flex-column align-items-center">
                                                <div class="avatar avatar-image flex-column align-items-center" style="width: 150px; height:150px">
                                                    <img src="{{asset('assets/images/avatars/'.$withdraw->user->profile_image)}}" alt="">
                                                </div>
                                            </div>
                                            <div class="  text-sm-left m-v-15 p-l-30 col-12 col-sm-8 flex-column">
                                                <h2 class="m-b-5 title-responsive">Requister: {{ $withdraw->user->name }}</h2>
                                                <p class="font-weight-semibold text-dark m-b-5">
                                                    <i class="m-r-10 text-primary anticon anticon-phone"></i>
                                                    <span class=" ">+{{ $withdraw->user->contact }}</span> 
                                                </p>
                                                <p class="font-weight-semibold text-dark m-b-5">
                                                    <i class="m-r-10 text-primary anticon anticon-mail"></i>
                                                    <span class=" ">{{ $withdraw->user->email }}</span> 
                                                </p>
                                                <p class="font-weight-semibold text-dark m-b-5">
                                                    <i class="m-r-10 text-primary anticon anticon-compass"></i>
                                                    <span class=" ">{{ $withdraw->user->status }}</span> 
                                                </p>
                                            </div>
                                        </div>
                                    </div>
    ...
    <form method="POST" action="{{route('withdraw.update', ['id' => $withdraw->id])}}"  enctype="multipart/form-data">
         @csrf
         @method('PUT')
         <div class="form-row">
        <div class="form-group col-md-6">
            <input type="number" hidden name="withdraw_id" value="{{$withdraw->id}}">
            <label for="withdraw_status" class="text-primary">Account Status</label>
            <select id="withdraw_status" class="form-control" name="withdraw_status">
                <option selected>Choose...</option>
                <option {{ 'pending' == $withdraw->status ? 'selected' : '' }} value="pending">Pending</option>
                <option {{ 'completed' == $withdraw->status ? 'selected' : '' }} value="completed">Completed</option>
            </select>
